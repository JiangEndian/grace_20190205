...
def cr(cvst, who, whenson, age, color, other='空白'):
    ...
    if who in ['君士坦丁堡会议']:
        cvst.create_text((bx+age/2+60)*kx, (by+nsy/2)*ky, text=msgt, font=ft, fill=colorto(color))
    elif True:
...
cvs = tk.Canvas(bibletime, width=cx, height=cy, bg='#fafafa', xscrollcommand=scrollbarx, yscrollcommand=scrollbary)#不加set,mint好使...
cvs.pack()
#对滚动条进行配置，滚动时触发什么
...
#cvs.create_text(300, 400, text=strdef(temp), font=fto, fill='black')
