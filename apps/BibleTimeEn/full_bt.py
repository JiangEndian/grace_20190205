...
def cr(cvst, who, whenson, age, color, other='空白'):
    ...
    if whenson == 0:
        msgt1 = 'AA%d-AA%d  Event:%s  Continue:%d' % (bAA,bAA+age, who, age)
    else:
        #msgt1 = 'AA%d-AA%d  Event:%s  Next:%d  Continue:%d' % (bAA,bAA+age, who, whenson, age)
        msgt1 = 'AA%d-AA%d  Event:%s  Continue:%d' % (bAA,bAA+age, who, age)
    time_ad_bc='' 
    if bBC > bAD() and (bBC-age) > bAD(age):#生死都在公元前。bBC不能=bAD()。bAD=1-bBC，不相等
        if len(other) < 16:
            msgt2 = 'BC%d-BC%d %s' % (bBC,bBC-age, other)
        else:
            msgt2 = 'BC%d-BC%d\n%s' % (bBC,bBC-age, other)
    elif bBC < bAD():#生于公元后 #BC0年时，已算为AD1年
        if len(other) < 16:
            msgt2 = 'AD%d-AD%d %s' % (bAD(),bAD(age), other)
        else:
            msgt2 = 'AD%d-AD%d\n%s' % (bAD(),bAD(age), other)
    else:#生于公元前，但死于公元后
        if len(other) < 16:
            msgt2 = 'BC%d-AD%d %s' % (bBC,bAD(age), other)
        else:
            msgt2 = 'BC%d-AD%d\n%s' % (bBC,bAD(age), other)
    #满足条件的给简化掉
    msgt = msgt1 + '  ' + msgt2
...
##输出至文本
    ...
    if True:
        ...
        cvst.create_text((bx)*kx, (by+nsy/2)*ky, text=msgt, anchor='w', font=ft, fill=colorto(color))
...
#cr(cvs, 'Event', Next, Continue, 'color', '大事记')
by += 1.5*sy
if True:#刚开始时画标尺
