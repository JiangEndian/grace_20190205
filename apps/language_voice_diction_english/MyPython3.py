...
###########################cvlc放音乐带快慢############
def cvlc_play_mp3(file_name, loops=1, key1='s', key2='f', key3='p', quitkey='q'):
    file_names = ''
    file_name = file_name.replace(' ', '\ ')
    for i in range(loops):
        file_names = file_names + file_name + ' '
    play_cmd = 'cvlc --global-key-quit \'%s\' --global-key-play-pause \'%s\' --global-key-rate-slower-fine \'%s\' --global-key-rate-faster-fine \'%s\' --play-and-exit %s > /dev/null 2>&1' % (quitkey, key3, key1, key2, file_names)
