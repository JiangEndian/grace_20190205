...
try:
    bookall = ''
    conn = sqlite3.connect('bible_korean_nkv.sqlite')
    cursor = conn.cursor()

    connc = sqlite3.connect('bible_chinese_sim.sqlite')
    ...
    for book in runsql(cursorc, 'select * from bible_chinese_sim'):
        bookcontent = book[1]
        bookno = book[2]
        chapterno = book[3]
        verseno = book[4]
        bookall = bookall + '\n' +  dictbible[bookno] + ' ' + str(chapterno) + ':' + str(verseno) + ' ' + str(bookcontent)
        
        cursor.execute('select * from bible_korean_nkv where c4book_no=? and c5chapter_no=? and c6verse_no=?', (bookno, chapterno, verseno))
        bookc = cursor.fetchall()
        if bookc:
            bookccontent = bookc[0][1] 
            bookall = bookall + '\n' + bookccontent
        else:
            bookall = bookall + '\n' + '这节韩语没有。。。'
        ...
        #print(bookall)
        #input()
    write2file('koreanchinesebible.txt', bookall)
finally:
    closedb(conn, cursor)
