...
def addEveryWeek():
    ...
    if not con_old:
        print('wordEmpty...')
        exit()
    con = right_heb(con_old)
    print('%s的显示对吗?' % con)
    env = input('mean:') #[ɪnˈvaɪrənmənt]环境
    #f_name = '%s/wavFile%s.wav' % (dir_name, str(voice_number))
    #f_name = '%s/%s' % (dir_name, input('输入hebrew_2018里的文件名数字:') + '.wav')
    f_name = '%s/%s.wav' % (dir_name, con_old)
    cvlc_play_mp3('~/grace_voice_file/%s' % f_name)
    input('确认对吗？')
    #voice_number += 1
